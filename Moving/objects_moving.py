...
class Particle_wsarm:
        ...
        for i, bl_cl in enumerate(block_cl):
            for case in enumerate(bl_cl):
                ...
                for j, val in enumerate(self.bl_att_cl[i]):
                    ...
                    block_distance[int(indexs_cl[j][0]):int(indexs_cl[j][1]+1), int(case[1][0]): int(case[1][1])+1, :] = val
    ...
    def process(self, type_draw='end', length = 10000):
        ...
        for t in range(0, length):
            ...
            if type_draw == 'end':
                if t == length - 1:
                    ...
                    ax[0].scatter(self.points_block_x, self.points_block_y)
                    for cl_raw in self.indexs_cl:
                        ax[0].scatter(self.points_obj_x[cl_raw[0]:cl_raw[1]+1], self.points_obj_y[cl_raw[0]:cl_raw[1]+1])
                    ax[1].plot(self.x_axis, self.force_loc_max, label="Force_loc_max")
                    ax[1].plot(self.x_axis, self.force_block_max, label="Force_block_max")
                    # ax[1].text(480, 10, t)
                    ax[1].legend()
